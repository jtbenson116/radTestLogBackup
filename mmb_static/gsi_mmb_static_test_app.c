...
// one for reading the regs in MMB.
GDL_TASK_DECLARE(populate_task);
GDL_TASK_DECLARE(read_mmb_task);
...
void writeLog(int max_iters, int iter_count, int vm_idx, int num_vmrs, int x_not_ok,
    ...
    char * datetime[20];
    sprintf(fname, "log_staticMMBtest_run%dlog%d_", masterRunIdx, numLogFiles);
...
int run_l1_serr(gdl_context_handle_t ctx_id, uint16_t *out1, uint16_t *in1,
 ...
 POPULATE:    
    ...
    // MAIN LOOP -----------------------------------------------------------------------------------
    // initialize vr_idx outside of the loop so we can access it for logging purposes.
    int vr_idx = 0;
	...
	if(userInput == 'r'){
		...
		for(int i=0; i < VR_SIZE; i++){
			...
			if(err_mask[vr_idx*VR_SIZE+i] == 0){
			    err_count++;
	    ...
	    if(x_not_ok){
		// Use this snippet to exit the test after an error
		/*
...
int main(int GSI_UNUSED(argc), char *argv[]){
    ...
    uint16_t *x;
    // Determine the number of vrs we want to check. the APU has 14 mmb vrs, so we
