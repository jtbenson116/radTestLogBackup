 ...
 *
 * rev1: Added wait for key press to read memory. Now we can press 'r' to read from
 * the GVML vm_regs on L1, 'd' to reload a fresh pattern, or 'q' to exit the program.
 *
 ...
 *
 */
// The printMatrix function is for debug and can be used for manual checking of vectors.
...
void writeLog(int max_iters, int iter_count, int vm_idx, int num_vmrs, int x_not_ok,
    ...
    char * datetime[20];
    sprintf(fname, "log_staticL1test_run%dlog%d_", masterRunIdx, numLogFiles);
...
// This method handles pretty much everything relating to the test.
...
int run_l1_serr(gdl_context_handle_t ctx_id, uint16_t *out1, uint16_t *in1,
    ...
    bool* err_mask = malloc(num_vmrs * VR_SIZE * sizeof(bool));
    // Reset error mask to zeros.
    for(int i=0; i<num_vmrs*VR_SIZE; i++)
	err_mask[i] = 0;
 ...
 POPULATE:    
	...
	printf("\nCumulative error count: %d", err_count);

	printf("\nPress 'r' to read, 'q' to exit, 'w' to write a log, or 'd' to reload a new pattern.");
	printf("\nWaiting for user input: ");
	scanf("%s", &userInput);
	//userInput = getchar();
	// If we input q, exit the loop and continue
	if(userInput == 'q'){
	    break;
	}
	...
	if(userInput == 'd'){
	    ...
	    for(int i=0; i<num_vmrs*VR_SIZE; i++)
		err_mask[i] = 0;
	    printf("Pattern reloaded.\n");
	    goto POPULATE;
	}

	// If we input 'r', then do the read process normally.
	if(userInput == 'r'){
	    ...
	    // what to do if we found any errors in them.
	    ...
	    if(x_not_ok){
		/*
		  printf("\nErrors detected after %d iterations. Exiting...\n", iter_count);
		  //goto POPULATE;
		  break;
		//*/
	    }
	    
	    // exit the loop if we have hit the max number of iterations as well. Max_iters
	    // is set as a command line argument to this program.
...
int main(int GSI_UNUSED(argc), char *argv[]){
    ...
    uint16_t *x;
    // Determine the number of vrs we want to check. the APU has 48 vmrs, so we
    // do not want to exceed that.
    uint16_t num_vmrs = 48;
